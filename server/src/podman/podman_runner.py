...
from . import windows_test_runner
...
def gen_requirements(path: str) -> bool:
    ...
    if should_run_in_container():
        ...
        lines_req_sub[:] = [line.split('==')[0].lower()
                            for line in lines_req_sub]
        ...
        # TODO: fix so we can get the requirements when
...
def run_container(image_name: str, test_dir: str, is_empty: bool) -> str:
    ...
    if should_run_in_container():
        ...
        return windows_test_runner.test_runner(Path(test_dir))
